...
const string SDB_PRIZE_LIST = "SDB_PRIZE_LIST";
const string TARGET = "SDB_PRIZE_TARGET";
const string CURRENT_PAGE = "SDB_PRIZE_PAGE";
const string PRIZE_COUNT = "SDB_PRIZE_COUNT";

const string PRIZE_TYPE = "SDB_PRIZE_TYPE";
...
void SetMenuOptionInt(string sOptionText, int iOptionValue, object oPC = OBJECT_SELF, string sList = SDB_PRIZE_LIST) {
   ReplaceIntElement(AddStringElement(sOptionText, SDB_PRIZE_LIST, oPC)-1, iOptionValue  , sList, oPC);
}
void SetMenuOptionString(string sOptionText, string sOptionValue, object oPC = OBJECT_SELF, string sList = SDB_PRIZE_LIST) {
   ReplaceStringElement(AddStringElement(sOptionText, SDB_PRIZE_LIST, oPC)-1, sOptionValue  , sList, oPC);
}
void SetMenuOptionObject(string sOptionText, object oOptionValue, object oPC = OBJECT_SELF, string sList = SDB_PRIZE_LIST) {
   ReplaceObjectElement(AddStringElement(sOptionText, SDB_PRIZE_LIST, oPC)-1, oOptionValue  , sList, oPC);
}
...
int GetPageOptionSelected(string sLIST = SDB_PRIZE_LIST) {
...
void BuildPage(int nPage) {
   ...
   object oTarget = GetLocalObject(oPC, TARGET);
   int nAwardCount = GetLocalInt(oPC, PRIZE_COUNT);
   DeleteList(SDB_PRIZE_LIST, oPC); // START FRESH PAGE
   switch (nPage) {
      case PAGE_MAIN_MENU:
      ...
      case PAGE_DO_PRIZE:
         string sMsg = "";
            sMsg = "Award " + GetName(oPC) + "\n\n   " + IntToString(nAwardCount) + AddS(" Bling Medallion", nAwardCount) + "?";
         SetDlgPrompt(sMsg);
...
void main () {
   ...
   switch(iEvent) {
      ...
      case DLG_PAGE_INIT:
         ...
         SetDlgResponseList(SDB_PRIZE_LIST, oPC);
         break;
        case DLG_SELECTION:
         HandleSelection();
         break;
