...
void BoostUndeadSummon(object oMaster, object oSum = OBJECT_INVALID) {
   ...
   int nPureLevel = GetLevelByClass(CLASS_TYPE_PALEMASTER, oMaster) + nPureBonus; // MAX 38
   if (GetHasFeat(FEAT_GREATER_SPELL_FOCUS_CONJURATION, oMaster)) nPureLevel++; // MAX 39
   if (GetHasFeat(FEAT_EPIC_SPELL_FOCUS_CONJURATION, oMaster)) nPureLevel++; // MAX 40
   ...
   if (IamSeed(oMaster)) FloatingTextStringOnCreature("Undead Boost Level " + IntToString(nBonus), oMaster, FALSE);
