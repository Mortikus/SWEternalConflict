...
void LoadItemProps(object oItem) {
   ...
   ItemProps.ItemType = InitCap(Get2DAString("baseitems", "label", ItemProps.ItemBase));
...
void ShowPropSelection(int iShow, int iUnique, string sText, int iAction, int iProp, int iAtMax = TRUE) {
   if (ItemProps.ValidProps & iShow) {
      ...
      if (iUnique) sText += GetColor(COLOR_GRAY) + " (unique)";
...
string FlagTooHigh(string sText, int iFlag) {
   return (iFlag) ? YellowText(sText) + RedText(" > Max!") + GetColor(TEXT_COLOR): sText;
}
...
string HeaderMsg() {
   ...
   int iTrade = 0;
   if (oCopy!=OBJECT_INVALID) iTrade = GetGoldPieceValue(oCopy);
...
//               AddNewProperty();
      ...
      case ACTION_BUY_ITEM:
         ...
         if (sText!="SEED_VALIDATED") {
            sText = "CRAFTED_" + SDB_GetSEID() + "_" + IntToString(IncLocalInt(GetModule(), "CRAFT_SEQ", 1));
      ...
      case PAGE_MENU_EQUIPPED:
         ...
         for(i = 0; i < INVENTORY_SLOT_ARROWS; i++) {
            ...
            if (GetIsObjectValid(oItem)) {
               if (GetTag(oItem)=="LEVELING_WEAPON" || GetTag(oItem)=="COO_AASSDARTS") {
                  SendMessageToPC(oPC, "Sorry, some weapons cannot be modified.");
               } else if (GetItemLevel(GetGoldPieceValue(oItem)) > MAX_LEVEL) {
                  AddMenuSelectionObject(FlagTooHigh(InventorySlotString(i) + ": " + GetName(oItem), TRUE), PAGE_MENU_EQUIPPED, oItem);
               } else {
                  AddMenuSelectionObject(InventorySlotString(i) + ": " + GetName(oItem), ACTION_COPY_ITEM, oItem);
               }
