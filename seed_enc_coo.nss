...
object MakeOutCast(int iLvl, object oLocation, int bFlyIn=TRUE, string sNewTag="") {
   object oMinion = MakeCreature(PickOutCast(iLvl), oLocation, bFlyIn, sNewTag);
...
void main() {
   ...
   // ***************
      ...
      if (!GetIsObjectValid(GetObjectByTag("COO_AASSVISION"))) {
         ...
         oMinion = MakeCreature("coo_aassvision", oLocator, TRUE);
   ...
   // ***************
      ...
      oMinion = MakeCreature("coo_chefemerald", oLocator, FALSE);
   }
      ...
      for (i=1; i<=k; i++) {
         if (d4()!=4) { // 75% chance of this seat taken
            ...
            if (d20()==1) CreateItemOnObject("COO_FLASKOFRUIN", oMinion, 1);
            HideWeapons(oMinion);
            HolySit(oMinion, oLocator);
         }
      }
      ...
      for (i=1; i<=2; i++) {
        ...
        oMinion = MakeCreature(sRef, oLocator, FALSE);
