...
void main() {
   ...
   if (sWhich=="LJ_BOSS_ENC") {
      ...
      for (i=0; i<3; i++) {
         ...
         oMinion = MakeCreature(PickOne("sauriaseer","sauriaseer001"), oLocator, FALSE);
         HideWeapons(oMinion);
      ...
      oMinion = MakeCreature("sauriafisherman", oLocator, FALSE);
      ...
      for (i=0; i<3; i++) {
         ...
         oMinion = MakeCreature("sauriaseer002", oLocator, FALSE);
         HideWeapons(oMinion);
         ...
         if (i<2) {
            oLocator = GetObjectByTag("ST_RACK", i);
