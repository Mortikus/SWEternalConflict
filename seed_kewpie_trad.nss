...
const int KEWPIE_XP_VALUE       = 100000; // 10k XP per doll
const int KEWPIE_XP_BONUS       = 0;     // 5% XP bonus for multiple dolls traded in
...
string DisabledText(string sText) {
   return GetColor(COLOR_GRAY) + sText;
}
...
int CountKewpies() {
   if (GetLocalInt(oPC, "IAMSEED")) return 6;
   int nKewpies = 0;
...
int TakeKewpies(int nKewpies, string sMsg) {
   ...
   while (GetIsObjectValid(oItem) && nKewpies) {
      ...
      if (GetStringLeft(sTag, 12)=="PRIZE_TOKEN_") {
         ...
         nKewpies--;
         string sSQL = "update tokentracker set tt_redemed=now(), tt_rplid="+SDB_GetPLID(oPC)+", tt_rmsg="+InQs(sMsg)+" where tt_ttid="+sTag;
         SQLExecDirect(sSQL);
         DestroyObject(oItem);
      }
      oItem = GetNextItemInInventory(oPC);
...
void ShowPrize(string sTag, int nCost, int nCount, int nAction) {
   string sPrize = IntToString(nCost) + AddS(" Kewpie", nCost) + ": ";
   ...
   if (sTag=="XP") {
      int nXP = (nCost * KEWPIE_XP_VALUE) + (KEWPIE_XP_VALUE * (nCost - 1)/7);
      nXP -= nXP % 500;
...
void ShowIOU(int bShowList=FALSE) {
   ...
   sSQL = "select " + sSQL + " from giveitem where gi_redemed is null and gi_ckid=" + SDB_GetCKID(oPC);
...
void HandleSelection() {
   ...
   switch (iOptionSelected) {
      ...
      case ACTION_BUY_ITEM:
         ...
         sText = "Trade " + IntToString(iOptionSubSelected) + AddS(" Kewpie", iOptionSubSelected) + " for ";
         if (bIsXP) {
      ...
      case ACTION_BUY_WINGS:
         SetBuyTag(sOptionSubSelected);
         sText = "Trade " + IntToString(iOptionSubSelected) + AddS(" Kewpie", iOptionSubSelected) + " for " + sOptionSubSelected + " Wings?";
         SetConfirmAction(sText, ACTION_BUY_WINGS, PAGE_MENU_MAIN);
         return;
      case ACTION_BUY_TAIL:
         SetBuyTag(sOptionSubSelected);
         sText = "Trade " + IntToString(iOptionSubSelected) + AddS(" Kewpie", iOptionSubSelected) + " for " + sOptionSubSelected + " Tail?";
      ...
      case ACTION_BUY_SKIN:
         ...
         sText = "Trade 6 Kewpies for the selected skin?\n\n   " + sOptionSelected;
      ...
      case ACTION_CONFIRM: // THEY SAID YES TO SOMETHING (OR IT WAS AUTO-CONFIRMED ACTION)
         ...
         switch (iConfirmed) {
            case ACTION_BUY_SKIN:
               ...
               if (!GetLocalInt(oPC, "IAMSEED")) SQLExecDirect("update skins set sk_plid=" + SDB_GetPLID(oPC) + " where sk_skid=" + IntToString(nCost));
               EndDlg();
               return;
            ...
            case ACTION_BUY_ITEM:
               ...
               if (nKewpies < nCost) {
                  ...
                  SDB_SetXP(oPC, nXPOnPC+nXP, "KEWPIEXP");
            ...
            case ACTION_CLAIM_IOU:
               {
                  ...
                  sSQL = "update giveitem set gi_redemed=now(), gi_plid="+SDB_GetPLID(oPC)+" where gi_giid=" + sGIID;
                  SQLExecDirect(sSQL);
...
void BuildPage(int nPage) {
   ...
   switch (nPage) {
      case PAGE_MENU_MAIN:
         ...
         SetPrompt(sText);
         AddMenuSelectionInt("XP" , PAGE_SELECT_XP);
         AddMenuSelectionInt("Damage Stones" , PAGE_SELECT_DAMAGE);
         AddMenuSelectionInt("Defensive Spell Stones" , PAGE_SPELL_DEFENSE);
         AddMenuSelectionInt("Offensive Spell Stones" , PAGE_SPELL_OFFENSE);
         if (nKewpies<2) AddMenuSelectionInt(DisabledText("Tails"), PAGE_MENU_MAIN);
         else            AddMenuSelectionInt("Tails" , PAGE_SELECT_TAIL);
         if (nKewpies<3) AddMenuSelectionInt(DisabledText("Wings"), PAGE_MENU_MAIN);
         else            AddMenuSelectionInt("Wings" , PAGE_SELECT_WINGS);
         if (nKewpies<6) AddMenuSelectionInt(DisabledText("Skins"), PAGE_MENU_MAIN);
         else            AddMenuSelectionInt("Skins" , PAGE_SELECT_SKIN);
         ShowIOU(FALSE);
         return;
      case PAGE_SELECT_XP:
      ...
      case PAGE_SELECT_DAMAGE:
         ...
         ShowPrize("SMS_DAM_NEGATIVE", 1, nKewpies, PAGE_SELECT_DAMAGE);
         ShowPrize("SMS_DAM_POSITIVE", 1, nKewpies, PAGE_SELECT_DAMAGE);
      ...
      case PAGE_SPELL_DEFENSE:
         ...
         ShowPrize("SMS_CAST_" + IntToString(IP_CONST_CASTSPELL_DEATH_WARD_7)                 ,20, nKewpies, PAGE_SPELL_DEFENSE);
         ShowPrize("SMS_CAST_" + IntToString(IP_CONST_CASTSPELL_ENERGY_BUFFER_20)             ,20, nKewpies, PAGE_SPELL_DEFENSE);
         ShowPrize("SMS_CAST_" + IntToString(IP_CONST_CASTSPELL_ETHEREAL_VISAGE_15)           ,20, nKewpies, PAGE_SPELL_DEFENSE);
         ShowPrize("SMS_CAST_" + IntToString(IP_CONST_CASTSPELL_FREEDOM_OF_MOVEMENT_7)        ,20, nKewpies, PAGE_SPELL_DEFENSE);
         ShowPrize("SMS_CAST_" + IntToString(IP_CONST_CASTSPELL_GLOBE_OF_INVULNERABILITY_11)  ,20, nKewpies, PAGE_SPELL_DEFENSE);
         ShowPrize("SMS_CAST_" + IntToString(IP_CONST_CASTSPELL_GREATER_SPELL_MANTLE_17)      ,20, nKewpies, PAGE_SPELL_DEFENSE);
         ShowPrize("SMS_CAST_" + IntToString(IP_CONST_CASTSPELL_IMPROVED_INVISIBILITY_7)      ,20, nKewpies, PAGE_SPELL_DEFENSE);
         ShowPrize("SMS_CAST_" + IntToString(IP_CONST_CASTSPELL_MIND_BLANK_15)                ,20, nKewpies, PAGE_SPELL_DEFENSE);
         ShowPrize("SMS_CAST_" + IntToString(IP_CONST_CASTSPELL_NEGATIVE_ENERGY_PROTECTION_15),20, nKewpies, PAGE_SPELL_DEFENSE);
         ShowPrize("SMS_CAST_" + IntToString(IP_CONST_CASTSPELL_PREMONITION_15)               ,20, nKewpies, PAGE_SPELL_DEFENSE);
         ShowPrize("SMS_CAST_" + IntToString(IP_CONST_CASTSPELL_PROTECTION_FROM_SPELLS_20)    ,20, nKewpies, PAGE_SPELL_DEFENSE);
         ShowPrize("SMS_CAST_" + IntToString(IP_CONST_CASTSPELL_SHADOW_SHIELD_13)             ,20, nKewpies, PAGE_SPELL_DEFENSE);
         ShowPrize("SMS_CAST_" + IntToString(IP_CONST_CASTSPELL_SPELL_RESISTANCE_15)          ,20, nKewpies, PAGE_SPELL_DEFENSE);
         ShowPrize("SMS_CAST_" + IntToString(IP_CONST_CASTSPELL_TRUE_SEEING_9)                ,20, nKewpies, PAGE_SPELL_DEFENSE);
         AddMenuSelectionInt("Back" , PAGE_MENU_MAIN);
         return;
      case PAGE_SPELL_OFFENSE:
         SetPrompt("Select the type of Offensive Spell Casting stone you'd like:");
         ShowPrize("SMS_CAST_" + IntToString(IP_CONST_CASTSPELL_BALAGARNSIRONHORN_7)          ,40, nKewpies, PAGE_SPELL_OFFENSE);
         ShowPrize("SMS_CAST_" + IntToString(IP_CONST_CASTSPELL_GREATER_DISPELLING_15)        ,40, nKewpies, PAGE_SPELL_OFFENSE);
         ShowPrize("SMS_CAST_" + IntToString(IP_CONST_CASTSPELL_GREATER_SPELL_BREACH_11)      ,40, nKewpies, PAGE_SPELL_OFFENSE);
         ShowPrize("SMS_CAST_" + IntToString(IP_CONST_CASTSPELL_PRAYER_5)                     ,40, nKewpies, PAGE_SPELL_OFFENSE);
         ShowPrize("SMS_CAST_" + IntToString(IP_CONST_CASTSPELL_SHAPECHANGE_17)               ,40, nKewpies, PAGE_SPELL_OFFENSE);
         ShowPrize("SMS_CAST_" + IntToString(IP_CONST_CASTSPELL_TENSERS_TRANSFORMATION_11)    ,40, nKewpies, PAGE_SPELL_OFFENSE);
         AddMenuSelectionInt("Back" , PAGE_MENU_MAIN);
         return;
      case PAGE_SELECT_WINGS:
         SetPrompt("Select the type of Wings you'd like:");
         ShowPrize("Angel",     3, nKewpies, PAGE_SELECT_WINGS);
         ShowPrize("Bat",       3, nKewpies, PAGE_SELECT_WINGS);
         ShowPrize("Bird",      3, nKewpies, PAGE_SELECT_WINGS);
         ShowPrize("Butterfly", 3, nKewpies, PAGE_SELECT_WINGS);
         ShowPrize("Demon",     3, nKewpies, PAGE_SELECT_WINGS);
         AddMenuSelectionInt("Back" , PAGE_MENU_MAIN);
         return;
      case PAGE_SELECT_TAIL:
         SetPrompt("Select the type of Tail you'd like:");
         ShowPrize("Bone",       2, nKewpies, PAGE_SELECT_TAIL);
         ShowPrize("Devil",      2, nKewpies, PAGE_SELECT_TAIL);
         ShowPrize("Lizard",     2, nKewpies, PAGE_SELECT_TAIL);
         AddMenuSelectionInt("Back" , PAGE_MENU_MAIN);
         return;
      case PAGE_SELECT_SKIN:
