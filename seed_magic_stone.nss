...
object oWorkContainer = GetObjectByTag("TavernInfo");    //
...
// Maximum Item Level
const int SMS_ITEM_LEVEL_MAX = 25;
...
void SMS_DelayTransferPower(object oPC, object oTarget, object oNew, object oItem, itemproperty ipAdd, int nVisual, string sMsg) {
   ...
   DestroyObject(oItem); // UNCOMMENT AFTER TEST TO GET RID OF ITEM AFTER USE!!
   ...
   if (bSuccess!=-1) {
      ...
      if (SQLFetch() != SQL_ERROR) {
         bSuccess = StringToInt(SQLGetData(1));
         if (bSuccess > 0) {
            SDBLogMsg("DUPSTONE", "Dup stone used " + GetName(oItem), oPC);
            sSQL = "update account set ac_dupflag=1 where ac_acid="+sACID;
            SQLExecDirect(sSQL);
...
int SMS_NextOnHit(object oPC, object oTarget, int nOnHitType, int nVisual) {
   ...
   if (nBonusPower==SMS_ON_HIT_DC_MAX) return SMS_PowerFailed(oPC, "This item has " + OnHitString(nBonusPower) + " " + sType + " On Hit bonus and cannot be enchanted any further.");
   nBonusPower = SMS_OnHitNext(nBonusPower);
   WriteTimestampedLogEntry("ONHIT: " + GetPCPlayerName(oPC) + "/" + GetName(oPC) + ": " + sType + " on hit increased to " + OnHitString(nBonusPower));
   return SMS_TransferPower(oPC, oTarget, ItemPropertyOnHitProps(nOnHitType, nBonusPower, IP_CONST_ONHIT_DURATION_50_PERCENT_2_ROUNDS), nVisual, sType+" on hit increased to "+OnHitString(nBonusPower));
}
...
//SendMessageToPC(oPC, "nBonusPower = " + IntToString(nBonusPower));
   ...
   SendMessageToPC(oPC, "This weapon now has " + IntToString(nBonusCnt) + " damage " + AddS("mod", nBonusCnt) + " and does a max of " + IntToString(nDamageTotal) + " damage. Max for this weapon is " + IntToString(WeaponStats.DamageMods) + " mods * " + IntToString(SMS_WEAPON_MODS_MAX) + " per mod = " + IntToString(nMaxDamage) + " damage.");
